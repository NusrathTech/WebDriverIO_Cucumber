class accountcreationPage {
  ...
  createAccount = async (emailId, address) => {
    ...
    // DOB
    await this.drpbx_Days.selectByIndex(20);
    await this.drpbx_Months.selectByAttribute("value", "5");
    await this.drpbx_Years.selectByAttribute("value", "1995");
    ...
    await this.input_address_City.setValue("Birmingham");
    await this.drpbx_State.selectByIndex(2);
    await this.input_PostalCode.setValue("35211");
    await this.txt_AdditionalInfo.setValue(
      "Test data to verify My additional Information "
    );
    await this.input_Phone.setValue("76543218");
    await this.input_MobilePhone.setValue("+1 8789373647");
    await this.input_AddressAlias.setValue("Sample Data");
